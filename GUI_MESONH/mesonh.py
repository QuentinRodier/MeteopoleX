...
class MesoNH:
    ...
    def __init__(self, date_run='latest', model_couplage='AROME',
                 type_forcage='MODEL', user_params={}):
        """
...
#            f = epygram.formats.resource('/cnrm/phynh/data1/rodierq/LEO/' + yyyymm_epy + '/historic.surfex.tlse-1300m000+0000:00_'+date_epygram+'.fa', 'r')
        ...
        for namelistgroup in self.user_params:
            NAM_ = "&NAM_"+namelistgroup+ " "
            if namelistgroup != 'id' and self.user_params[namelistgroup] is not None:
                for params in self.user_params[namelistgroup]:
                    NAM_ += params + "=" + str(self.format2namelist(self.user_params[namelistgroup][params])) + ", "
                NAM_ += "/" # End of the namelist group
            namelist = namelist.replace("{&NAM_"+namelistgroup+"}", NAM_)
            namelist = namelist.replace(", /","/") #remove the last , before ending a namelist group
       ...
       """
        ...
        if f.getncattr('MNH_cleanly_closed') == 'yes':
            if 'id' in self.user_params:
                ...
                id_run = ''
            output_file = self.path_output + 'MESONH_' + self.model_couplage_aida + \
                '_' + self.date_run.strftime('%Y%m%d%H') + id_run
            call(
                'ncrcat -O ' +
                self.path +
                'REF01.1.SEG01.*[1-9].nc ' +
                output_file +
                '.nc',
                shell='True')
            call(
...
#        call('rm -rf ' + self.path, shell='True')
    ...
    def get_initialize_file(self, model_couplage, date_run):
        ...
        if model_couplage == "AROME":
            ...
            file = '/cnrm/ktrm/manip/METEOPOLEX/AROME/Toulouse/' + yyyymm_aro + \
                '/netcdf_tlse.tar.arome_' + date_run_arome + '.netcdf'
#            file = '/cnrm/phynh/data1/rodierq/LEO/' + yyyymm_aro + '/netcdf_tlse.tar.arome_'+date_run_arome+'.netcdf'
            ...
            file = '/cnrm/proc/bazile/NetCdf/Toulouse/' + yearmonth + \
                '/Arpege-oper-L105_Toulouse_' + date_run_arpege + '.nc'
    ...
    def check_user_params(self, user_params):
        ...
        if isinstance(user_params, dict):
            ...
            raise ValueError(
                'Les paramètres doivent être fournis sous forme d\'un dictionnaire {clé: valeur}')
    ...
    def create_NAM_IDEAL_FLUX(self):
        ...
        for i, time in enumerate(times[:ech_max]):
            ...
            time = float((time - self.date_run).days) * 86400 + \
                float((time - self.date_run).seconds) - 1800
        ...
        for id_aida in dico_ids_aida:
            if id_aida == "CO2":
                ...
                values, times, header = read_aida.donnees(
                    doy1, doy2, year, "flx_mvt_Chb_%1800", "Tf")
                values = np.sqrt(values / self.rho)
            elif id_aida == "LE":
                vec_le, times, header = read_aida.donnees(
            ...
            for i, value in enumerate(values[:ech_max]):
                ...
                # On réajuste la valeur de i (i+2) et on prend la même valeur que pour i=3
                # pour les échéances i=1 (OOh) et i=2 (OOh15)
                if i == 0:

                    txt += "X" + dico_ids_aida[id_aida] + \
                        "(" + str(1) + ")=" + str(values[0]) + ",\n"
                    txt += "X" + dico_ids_aida[id_aida] + \
                        "(" + str(2) + ")=" + str(values[0]) + ",\n"
        ...
        if len(vec_le.mask.nonzero()[0]) > 0:
            for i in range(len(vec_le)):
                if not 6 < int(time[i].strftime('%H')) < 22:
                    if not isinstance(vec_le[-i], np.float32):
                        vec_le[-i] = 0

                if vec_hs[i] <= 0 and not isinstance(vec_le[i], np.float32):
                    vec_le[i] = 0
