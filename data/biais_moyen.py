    ...
    for param in VARIABLES_PLOT:
        for model in MODELS_CONFIG.keys():
            for reseau in RESEAUX:
                ...
                # Concat de tous les runs de ce modèle/réseau/param
                combined = pd.concat(series_list, axis=0).sort_index()
                combined = combined.groupby(combined.index).mean()  # dédoublonner
                col_data[colname] = combined

    # Construire le DF global en alignant par index (timestamp)
    if col_data:
        DF = pd.DataFrame(col_data)  # alignement automatique par index
        DF = DF.apply(pd.to_numeric, errors='coerce')
        DF_CyDi = DF.groupby(DF.index.hour).mean()
    else:
        DF_CyDi = pd.DataFrame()

    # Construction du dictionnaire biais_moy
    for param in VARIABLES_PLOT:
        ...
        for model in MODELS_CONFIG.keys():
            ...
            for reseau in RESEAUX:
                ...
                if colname in DF_CyDi.columns:
                    ...
                    biais_moy[param][model][reseau]["values"] = [np.nan] * max(len(DF_CyDi), 24)
                    biais_moy[param][model][reseau]["time"] = list(range(max(len(DF_CyDi), 24)))
