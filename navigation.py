...
# Le 'title' est le titre du graph qui sera affiché, donc c'est
# l'utilisateur qui choisi, idem pour le unit, c'est à l'utilisateur de
# rentrer les bonnes unités.
...
# C'est la liste de tous les paramètres qui vont être tracés. Pour toute
# modification de cette liste, penser à modifier le dico_params en
# conséquence.
...
@app.callback(output_graphs,
              [Input('multi_select_line_chart_obs', 'value'),
               ...
               Input(
                  'id_user1', 'value'), Input(
                  'id_user2', 'value'), Input(
                  'id_user3', 'value'), Input(
                  'id_user4', 'value'), Input(
                  'id_user5', 'value')
               ])
...
# Il faut donc parcourir tous les jours entre le start_day et le end_day
# et stocker les informations lues.
    courbe_affichee = []
    for selection in reseau4:
        ...
        for i in range(nb_jour + 1):
            ...
            if selection not in courbe_affichee and dico_model[selection]['name'] in data_mnh[today_str] and isinstance(
                    data_mnh[today_str][dico_model[selection]['name']]['tmp_2m'], (list, np.ndarray)):
...
content = html.Div([
    ...
    html.Div([html.Span('"Nom-du-modèle_date-de-run"',
    ...
    html.Div([html.Span('MesoNH-Aro',
                        style={"font-weight": "bold"}),
              html.Span(' est forcé par Arome 0h du jour.')],
             className="twelve columns",
             style={"text-align": "center",
                    "justifyContent": "center"}),
    html.Div([html.Span('MesoNH-Arp',
                        style={"font-weight": "bold"}),
    ...
    html.Div([html.Span('SURFEX_Aro',
                        style={"font-weight": "bold"}),
              html.Span(' est forcé par Arome 00h du jour.')],
             className="twelve columns",
             style={"text-align": "center",
                    "justifyContent": "center"}),
    html.Div([html.Span('SURFEX_Arp',
                        style={"font-weight": "bold"}),
              html.Span(' est forcé par Arpège 00h du jour.')],
             className="twelve columns",
             style={"text-align": "center",
                    "justifyContent": "center"}),
    html.Div([html.Span('SURFEX_Obs',
                        style={"font-weight": "bold"}),
    ...
    html.Br(),
    html.Span(
        ' Sans sélection de la part de l’utilisateur, un paramètre prendra sa valeur par défaut (cf '),
    html.A(
        "la documentation utilisateur de MésoNH",
        href='http://mesonh.aero.obs-mip.fr/mesonh54/BooksAndGuides?action=AttachFile&do=get&target=users_guide_m544.pdf#subsection.9.2.44)'),
    html.Span(').'),
    html.Br(),
